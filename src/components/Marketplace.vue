                            ...
                            <div class="bidding-container" v-bind:style="'background: #' + constellation.owner.substr(constellation.owner.length - 6) ">
                                <div class="buy-constellation">
                                    <div class="buy-container" >
                                        <a>{{ constellation.priceEth.substr(0, 6) }} ETH</a>
                                        <button v-on:click="buyitem(index, constellation.priceWei)">Buy</button>
                                    </div>
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    beforeCreate: function () {
        ...
        CryptoConstellation.init().then(() => {
            CryptoConstellation.totalSupply(window.web3.eth.accounts[0]).then((totalTokens) => {
                console.log(totalTokens)
                for(let token_id=0; token_id <= Number(totalTokens); token_id++){
                    
                    console.log(token_id)
                    CryptoConstellation.getConstellation(token_id).then(result => {
                        ...
                        this.constellations.push({
                            ...
                            ipfsHash: result[2],
                            owner: result[3],
                            startingPrice: Number(result[4]),
                            priceEth: window.web3.fromWei(Number(result[5]), 'ether'),
                            priceWei: Number(result[5]) + 1,
                            nextPrice: Number(result[6])
                        })
    ...
    methods: {
        ...
        buyitem: function (index, priceWei) {
            console.log(index)
            CryptoConstellation.buy(index, priceWei).then((tx) => {
                console.log(tx)
            })
        }
    }
